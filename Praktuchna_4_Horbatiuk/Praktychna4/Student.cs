...
public class Student : ICloneable
{
    ...
    public List<GradePoint> GradePoints { get; init; } = new();
    ...
    // Оператори порівняння студентів за середнім балом.
    ...
    // Оператор + : об'єднання двох студентів у "командний" профіль.
    public static Student operator +(Student a, Student b)
    {
        string surnameA = a.FullName.Split(' ')[0];
        string surnameB = b.FullName.Split(' ')[0];

        var team = new Student
        {
            FullName = $"Команда {surnameA} та {surnameB}",
            RecordBookNumber = "00000000",
